...
public interface InternshipAddressRepository extends JpaRepository<InternshipAddress, Integer> {
    ...
    @Query("select i from InternshipAddress i where i.address.company.user.id = ?1 and i.internship.status = ?2")
    List<InternshipAddress> findInternshipAddressesBy(Integer userId, String status);
